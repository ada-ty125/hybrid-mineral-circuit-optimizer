...
int main() {
  ...
  check_close("1. empty + 1A -> op cost only",
              economic_value(zero, zero, circuit_1A), -10.0);

  // 2) empty streams + 7A + 3B -> -10*7 - 12*3 = -106 GBP/s
  check_close("2. empty + 7A+3B -> op cost only",
              economic_value(zero, zero, circuit_7A_3B), -106.0);

  // 3) pal_product = {1,0,0}, no units -> +120 GBP/s
  {
    Stream pal{1.0, 0.0, 0.0};
    check_close("3. pal product = 1 kg/s palusznium",
                economic_value(pal, zero, circuit_0), 120.0);
  }
  ...
  // 5) pal_product = {0,0,1}, no units -> -500 GBP/s
  {
    ...
    check_close("5. pal product = 1 kg/s waste (penalty)",
                economic_value(pal, zero, circuit_0), -500.0);
  }
  ...
  // 7) gor_product = {0,1,0}, no units -> +100 GBP/s
  {
    ...
    check_close("7. gor product = 1 kg/s gormanium",
                economic_value(zero, gor, circuit_0), 100.0);
  }

  // 8) worst_case_value(80) -> -40000 GBP/s (base feed waste rate)
  check_close("8. worst-case fitness (waste = 80 kg/s)",
              worst_case_value(80.0), -40000.0);
  ...
  // 9) Same inputs twice must give the same bits (no hidden state).
  {
    ...
    check_bits_equal("9. determinism: repeat call returns identical bits", a,
                     b);
  }
  ...
  //     net     = 492.75 - 106 = 386.75
  {
    ...
    check_close("11. realistic mixed case",
                economic_value(pal, gor, circuit_7A_3B), 386.75);
  }
