...
void BM_EconomicValue_FixedOpCost_Empty(benchmark::State& state) {
  for (auto _ : state) {
    double v =
        economic_value(kEmpty, kEmpty, kCircuit_7A_3B, &fixed_op_cost,
                       default_economics);
...
void BM_EconomicValue_FixedOpCost_Realistic(benchmark::State& state) {
  for (auto _ : state) {
    double v = economic_value(kRealisticPal, kRealisticGor, kCircuit_7A_3B,
                              &fixed_op_cost, default_economics);
    benchmark::DoNotOptimize(v);
  }
}
...
void BM_Batch1000(benchmark::State& state) {
  for (auto _ : state) {
    ...
    for (int i = 0; i < 1000; ++i) {
      acc += economic_value(kRealisticPal, kRealisticGor, kCircuit_7A_3B,
                            &fixed_op_cost, default_economics);
    }
...
void write_csv(const RowBuffer& buf, double max_err) {
  ...
  const std::string path =
      dir + "/economics_" + iso8601_compact_for_filename() + ".csv";
  ...
  for (const auto& r : buf.runs) {
    ...
    const std::string model =
        it != g_op_cost_model.end() ? it->second : "fixed_op_cost";
    // Benchmark gives total seconds; we want ns per iteration in the CSV.
    const double iters = static_cast<double>(r.iterations);
    const double real_ns = iters > 0 ? (r.real_accumulated_time / iters) * 1e9
                                     : 0.0;
    const double cpu_ns = iters > 0 ? (r.cpu_accumulated_time / iters) * 1e9
                                    : 0.0;
    out << ts << "," << CUPRITE_GIT_SHA << "," << name << "," << model << ","
        << r.iterations << "," << real_ns << "," << cpu_ns << ","
        << std::setprecision(17) << max_err << ",baseline\n"
        << std::setprecision(6);
  }
...
int main(int argc, char** argv) {
  ...
  register_op_cost_model("BM_EconomicValue_FixedOpCost_Empty",
                         "fixed_op_cost");
  register_op_cost_model("BM_EconomicValue_FixedOpCost_Realistic",
                         "fixed_op_cost");
