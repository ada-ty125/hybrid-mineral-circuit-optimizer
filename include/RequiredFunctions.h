/**
 * @file RequiredFunctions.h
 * @brief Shared public interfaces for the genetic algorithm, simulator, and visualisation modules.
 *
 * This header declares the project-level functions used to connect candidate
 * generation, optimisation, circuit simulation, validity checking, and
...
void seed_rngs(
    unsigned seed);  // to seed next random number generation, defined in libGeneticAlgorithm
...
// which can be used in the optimize_span function. You can also implement other helper functions as
// needed.

// a function which generates a random vector of integers
void default_generator(std::span<const int> const fixed_prefix, std::span<int> values,
                       RandomNumberGenerator& rng);
// a function which mutates a random vector of integers
void default_mutator(std::span<const int> const fixed_prefix, std::span<int> values,
                     std::size_t max_mutations, RandomNumberGenerator& rng);
...
void default_crossover(std::span<const int> const fixed_prefix,
                       ...
                       std::span<std::span<int>> child_values, RandomNumberGenerator& rng);
// similarity function to calculate the similarily between two vectors
double default_similarity(std::span<const int> const fixed_prefix, std::span<const int> const vec1,
...
struct GA_Functions {
    ...
    // a function which generates a random vector of integers
    void (*vector_generator)(std::span<const int> const fixed_prefix, std::span<int> values,
                             RandomNumberGenerator& rng) = &default_generator;
    // a function which mutates a random vector of integers
    void (*vector_mutator)(std::span<const int> const fixed_prefix, std::span<int> values,
...
BaseResult<std::span<int>> optimize_span(
    ...
    GA_Functions
        ga_functions);  // Optimize a random vector of integers with some fixed values and some
                        // random values, using a genetic algorithm. defined in libGeneticAlgorithm
...
// The function which calculates the overall performance, defined in libSimulator.
...
bool check_validity(
    std::span<const int> const circuit_span);  // A fast function which applies graph heuristics to
                                               // rule out invalid vectors. defined in libSimulator
...
void generate_circuit_vector(
    std::span<const int> const fixed_values, std::span<int> random_values,
    RandomNumberGenerator& rng);  // Generate a random vector of integers with some fixed values and
                                  // some random values, defined in libSimulator

void mutate_circuitvector(
    std::span<const int> const fixed_values, std::span<int> mutable_values,
    RandomNumberGenerator& rng);  // Mutate a random vector of integers with some fixed values and
                                  // some random values, defined in libSimulator

void crossover_circuit_vectors(
    std::span<const int> const fixed_values, std::span<const int> const parent1_values,
    std::span<const int> const parent2_values,
    std::span<std::span<int>>
        child_values,  // Length of outer span is the number of children, length of inner spans is
                       // the length of the random part of the vector
    RandomNumberGenerator&
        rng);  // Cross over two random vectors of integers with some fixed values and some random
               // values to produce a child vector, defined in libSimulator
 ...
 *
 * The current visualisation implementation supports one input stream, three
 * product streams, and unit output counts of two or three. Malformed vectors
