/**
 * @file CUnit.h
 * @brief Defines the CUnit class representing an individual separation unit in a mineral processing circuit.
...
class CUnit {
  public:
    ...
    ///@{
    double feed[N_COMPONENTS] = {0.0, 0.0, 0.0}; /**< Current aggregate feed array entering this unit. Index map: [0]=P, [1]=G, [2]=W. */
    double feed_P = 0.0; /**< Convenient individual tracker for current incoming Palusznium mass flow rate. */
    double feed_G = 0.0; /**< Convenient individual tracker for current incoming Gormanium mass flow rate. */
    double feed_W = 0.0; /**< Convenient individual tracker for current incoming Waste mass flow rate. */
    ///@}

    /** @name Previous Iteration Feed States (for Convergence Checks) */
    ///@{
    double old_feed[N_COMPONENTS] = {0.0, 0.0, 0.0}; /**< Feed mass array recorded from the previous solver iteration. */
    double old_feed_P = 0.0; /**< Previous iteration tracker for incoming Palusznium mass flow rate. */
    double old_feed_G = 0.0; /**< Previous iteration tracker for incoming Gormanium mass flow rate. */
    ...
    double tails[N_COMPONENTS] = {0.0, 0.0, 0.0}; /**< Tailings exit stream vector carrying unrecovered remnants out of the unit. */

    /** @name Graph Traversal & Validity Flags */
    ///@{
    bool mark = false;               /**< Visited flag used primarily during recursive graph traversal checks. */
    bool reaches_palusznium = false; /**< Evaluation flag tracking if this cell's output pathway can reach the final Palusznium stream. */
    bool reaches_gormanium = false;  /**< Evaluation flag tracking if this cell's output pathway can reach the final Gormanium stream. */
    bool reaches_tailings = false;   /**< Evaluation flag tracking if this cell's output pathway can reach the final aggregate tailings exit. */
    ///@}

    /** @name Cell Physical Constants & Kinetics */
    ///@{
    double volume = 10.0;             /**< Total operational volumetric capacity of the cell tank. */
    double phi = 0.1;                 /**< Constant multiplier or structural physical coefficient (e.g., gas holdup or fraction parameter). */
    double rho = 3000.0;              /**< Volumetric density constant representing solid material components. */
    mutable double residence_time = 0.0; /**< Calculated fluid stay period within the tank. Marked mutable for lazy-evaluation tracking inside const methods. */
    ///@}
     ...
     * @return Double precision value representing total mass flow (\f$ \text{Feed}_P + \text{Feed}_G + \text{Feed}_W \f$).
     */
    double total_feed() const;

    /**
     * @brief Calculates and updates the internal residence time of the cell based on current throughput metrics.
     ...
     * @param component Targeted mineral column selector index (0 = Palusznium, 1 = Gormanium, 2 = Waste).
     * @param k_matrix Reference array representing kinetic multiplier variables for Type A and Type B units.
    ...
    /**
     * @brief Executes the simulation separation laws to compute and distribute current material into concentrate and tailings structures.
