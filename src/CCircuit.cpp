...
bool check_validity(const ESE::CSRGraph& graph) {
    ...
    for (int i = 0; i < num_units; ++i) {
        ...
        for (int j = 0; j < circuit.units[i].n_outputs; ++j) {
            circuit.units[i].output[static_cast<std::size_t>(j)] =
                static_cast<int>(graph.node_targets(i, j));
        }
    }
...
Circuit::Circuit() : num_inputs_(0), num_units_(0), num_products_(0), feed_dest_(-1) {}
...
bool Circuit::initialise(std::span<const int> circuit_span) {
    ...
    // Layout: [num_inputs, num_units, num_products, per-unit output counts..., feed_dest,
    // destinations...].
    ...
    for (std::size_t unit_id = 0; unit_id < num_units_size; ++unit_id) {
        ...
        for (int output_id = 0; output_id < unit.n_outputs; ++output_id) {
            unit.output[static_cast<std::size_t>(output_id)] = circuit_span[destination_index++];
        }
    }
...
int Circuit::product_index(int product_id) const noexcept { return product_id - num_units_; }
...
int Circuit::num_units() const noexcept { return num_units_; }

int Circuit::num_products() const noexcept { return num_products_; }
